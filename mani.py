...
class vgg_upper(object):
    ...
    def vgg_trans(self, X, X_t, y, Lap):
        # h = self.max_pool(tf.nn.relu(X), 'pool4')
        # t = self.max_pool(tf.nn.relu(X_t), 'pool4')
        h, t = X, X_t
        for i in xrange(1, 5):
            name = 'conv5_%d'%i
        ...
        # loss
        '''
        sz = self.Lap.get_shape().as_list()[0]
        tt = tf.tile(tf.reshape(t, (1, sz, -1)), [sz, 1, 1])
        tt -= tf.transpose(tt, [1, 0, 2])
        tt = tf.reduce_sum(tf.mul(tt, tt), 2)
        mani = tf.reduce_sum(tf.mul(self.Lap, tt))
        '''
